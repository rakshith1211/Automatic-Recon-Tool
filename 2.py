...
def format_shodan(sdata):
    lines = ["\n=== Shodan Info ==="]
    if "error" in sdata:
        lines.append(sdata["error"])
    else:
        lines.append(f"IP: {sdata.get('ip', 'N/A')}")
        lines.append(f"Organization: {sdata.get('org', 'N/A')}")
        lines.append(f"ISP: {sdata.get('isp', 'N/A')}")
        lines.append(f"ASN: {sdata.get('asn', 'N/A')}")
        lines.append(f"Open Ports: {', '.join(map(str, sdata.get('ports', []))) or 'None'}")
        lines.append(f"Vulnerabilities (first 30): {', '.join(sdata.get('vulns', [])) or 'None'}")
...
class ReconGUI:
    ...
    def build_exe(self):
        result = messagebox.askyesno(
            "Build Executable",
            "This will use PyInstaller to create a standalone executable. This requires PyInstaller to be installed (`pip install pyinstaller`). Continue?"
        )
        if not result:
            return
        ...
        try:
            ...
            # Create a simple spec file to bundle the script and the output folder.
            spec_content = f'''
...
'''
            ...
            # Using a separate thread to prevent GUI freeze
            build_thread = threading.Thread(target=self._run_pyinstaller, args=(spec_path,))
            build_thread.daemon = True
            build_thread.start()
            
        except FileNotFoundError:
            messagebox.showerror("Error", "PyInstaller is not installed or not in PATH. Please run 'pip install pyinstaller'.")
        except Exception as e:
    ...
    def _run_pyinstaller(self, spec_path):
        try:
            ...
            if proc.returncode == 0:
                ...
                self.safe_append(f"\nBuild successful! Executable is located at: {dist_path}\n")
                messagebox.showinfo("Success", f"Build successful! Executable is located at: {dist_path}")
            else:
